...
(function (window, undefined) {
  ...
  EpicEditor.prototype.load = function (callback) {
    ...
    _goSplitScreen = function () {

      _isInEdit = self.eeState.edit;

      // Set the state of EE in splitScreen
      // We set edit and preview to true also because they're visible
      self.eeState.splitScreen = true;
      self.eeState.edit = true;
      self.eeState.preview = true;

      console.log($(self.settings.container))

      // Cache calculations
      var containerOuterWidth = _outerWidth(self.settings.container)
        , containerOuterHeight = _outerWidth(self.settings.container);

      console.log(containerOuterWidth, containerOuterHeight);

      // This MUST come first because the editor is 100% width so if we change the width of the iframe or wrapper
      // the editor's width wont be the same as before
      _elementStates.editorIframe = _saveStyleState(self.editorIframe, 'save', {
        'width': containerOuterWidth / 2 + 'px'
    ...
    // This setups up live previews by triggering preview() IF in fullscreen or splitScreen on keyup
    self.editor.addEventListener('keyup', function () {
      if (keypressTimer) {
        window.clearTimeout(keypressTimer);
      }
      keypressTimer = window.setTimeout(function () {
        if (self.eeState.fullscreen || self.eeState.splitScreen) {
          self.preview();
        }
      }, 250);
    });
    ...
    function shortcutHandler(e) {
      ...
      if (isMod === true && e.keyCode == self.settings.shortcut.preview && !self.eeState.fullscreen && !self.eeState.splitScreen) {
        e.preventDefault();
        self.preview();
      }
    ...
    window.addEventListener('resize', function () {
      ...
      else if (!self.eeState.fullscreen && !self.eeState.splitScreen) {
        resetWidth(elementsToResize);
      }
    });

    if (self.settings.splitScreen) {
      _goSplitScreen();
    }
