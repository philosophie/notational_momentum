...
(function (window, undefined) {
  ...
  EpicEditor.prototype.load = function (callback) {
    ...
    self._splitscreen = function () {

      _isInEdit = self.is('edit');

      // Set the state of EE in splitScreen
      // We set edit and preview to true also because they're visible
      self._eeState.splitscreen = true;
      self._eeState.edit = true;
      self._eeState.preview = true;

      // Cache calculations
      var containerOuterWidth = _outerWidth(self.settings.container)
        , containerOuterHeight = _outerHeight(self.settings.container);
    ...
    self.editor.addEventListener('keyup', function () {
      ...
      keypressTimer = window.setTimeout(function () {
        if (self.is('fullscreen') || self.is('splitscreen')) {
          self.preview();
        }
      }, 250);
    });
    ...
    function shortcutHandler(e) {
      ...
      if (isMod === true && e.keyCode == self.settings.shortcut.preview && !self.is('fullscreen') && !self.is('splitscreen')) {
        e.preventDefault();
        if (self.is('edit')) {
          self.preview();
        }
  ...
  EpicEditor.prototype.is = function (what) {
    ...
    case 'splitscreen':
      return self._eeState.splitscreen;
    default:
      return false;
    }
  }
